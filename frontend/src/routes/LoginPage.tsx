    ...
    <div className="relative min-h-screen overflow-hidden flex items-center justify-center p-6 bg-surface-base">
      ...
      >
        <div className="flex flex-col items-center mb-7">
          ...
          <h1 className="text-ink-primary text-h2 font-bold tracking-tight">{t("로그인", "Sign in")}</h1>
        </div>
        ...
        <form onSubmit={submit} className="flex flex-col gap-3.5">
          ...
          <div className="flex items-center justify-between px-1 text-meta">
            ...
            <button type="button" className="text-ink-secondary hover:text-brand-primary transition-colors" onClick={() => setHint((h) => !h)}>
              {t("비밀번호를 잊으셨나요?", "Forgot Password?")}
            </button>
          </div>

          {hint && (
            <div className="text-meta text-ink-muted text-center -mt-1">
              {t("계정은 관리자가 발급합니다. 관리자에게 문의하세요.", "Accounts are provisioned by an admin — please contact them.")}
            </div>
          )}
